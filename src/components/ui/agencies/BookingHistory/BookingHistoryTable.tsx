...
import { useGetMyBookingHistoryQuery, useManageBookingsMutation } from "@/redux/api/agencyApi"
...
const MyPlans = ({ id }: { id: number }) => {
        ...
        <>
              {
                  data.status == 'booked' ?
                      <div style={{ display: "flex", gap: "5px" }}>
                          <Button danger onClick={()=>handleBookings(data?.id,'cenceled')}>
                          ...
                          <div>
                            ...
                            <div style={{ display: "flex", gap: "5px" }}>
                                <Button type="primary" onClick={()=>handleBookings(data?.id,'booked')}>
                                Accept Booking</Button>
                                    <Button danger onClick={()=>handleBookings(data?.id,'rejected')}>
                                Reject</Button>
                                </div>
                 }
  ...
  return (
    <>
          <Card>
              <span style={{display:'flex',alignItems:"center",gap:'10px', margin:"10px 0"}}>Plan Title: <h3> { data?.plan_name}</h3></span>
      <Table
        columns={columns}
