  ...
  return (
    <div>
      ...
      {/* mobile layout */}
        ...
        <Card style={{ width: '100%' }}>
                <Avatar style={{backgroundColor:'var(--secondary-color)'}} size={128} icon={ 
                data?.profile_img ? <Image src={data?.profile_img} width={100} height={100} layout="responsive" alt="dummy" /> :
                  <Image src={dummy_img} style={{borderRadius:"20px"}} width={100} height={100} layout="responsive" alt="dummy"/>
            } />
            ...
            <h1 style={{margin:"20px 0"}}> <span style={{textTransform:"uppercase"}}>{data?.first_name} { data?.last_name}</span></h1>
            <hr style={{margin:"20px 0"}}/>
            <h3>Email address:</h3> <span>{data?.email}</span>
            <div><h3>Contact no</h3> <span>{data?.contact_no}</span></div>
            <div>
