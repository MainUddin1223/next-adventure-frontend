...
import { useGetPayoutsQuery, usePayoutRelaseMutation } from '@/redux/api/adminApi';
...
//   };
  ...
  const columns: TableColumnsType<DataType> = [
    ...
    {
      title: 'Payouts', key: 'name',
      render: (data) => {
        return (
          <div>
            {
              data.groupedPayouts.length ? data.groupedPayouts.map((payoutsData: any, i: number) => (
                payoutsData?.status == 'pending' &&
                  <h4 key={i}>{payoutsData?.status} <strong>{(payoutsData?.totalAmount.toFixed(2))}</strong></h4> 
          ...
          <div>
            {
              data.groupedPayouts.length ? data.groupedPayouts.map((payoutsData: any, i: number) => (
                payoutsData?.status == 'pending' &&
                  <Button type='primary' key={i} onClick={()=>handlePayouts(data.id)}>Pay now</Button> 
