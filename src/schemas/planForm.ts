...
export const planFormSchema = yup.object().shape({
    ...
    cover_location: yup.array(yup.string().required()).min(1).required('Cover location is required'),
    events: yup.array(yup.string().required()).min(1).required('Events are required'),
})
